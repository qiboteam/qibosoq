        ...
        for piece in self:
            for offset, line in enumerate(piece.split("\n")):
                if ("print" in line) and ("CodeText:skip" not in line):
                    ...
                    raise ValueError(f"Found `{target_word}` in line {line_num} of {self.filedir}.")


def python_files():
    """Iterator that yields all python files (`.py`) in `/src/qibosoq/`."""
    basedir = pathlib.Path(qibosoq.__file__).parent.absolute()
...
def test_qibosoq_code(filename):
